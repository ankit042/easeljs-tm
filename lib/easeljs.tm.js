(function() {
  ...
  createjs.tm.Graphics = (function(_super) {
    var Container, Shape, Stage, Xor128;
    ...
    Graphics.prototype._perlineNoise = function(width, height, baseX, baseY, numOctaves, randomSeed, stitch, fractalNoise, channelOptions, grayScale, offsets) {
      var gaussianFilter, h, i, j, k, noisedPixels, octave, octaves, offset, persistence, persistences, pixel, pixels, scale, scaledPixels, targetPixels, totalWeight, w, weight, _i, _j, _k, _l, _len, _len1, _len2, _len3, _len4, _m, _n, _o, _ref;
      if (channelOptions == null) {
        channelOptions = 7;
      }
      if (grayScale == null) {
      ...
      for (i = _k = 0; _k < numOctaves; i = _k += 1) {
        ...
        noisedPixels = this._noise(w, h, randomSeed, 0, 0xff, channelOptions, grayScale, offset);
        scaledPixels = this._scale(noisedPixels, w, h, width, height, i);
        gaussianFilter.filter(width, height, scaledPixels, octave.pixels);
      }
      targetPixels = [];
      for (i = _l = 0, _len1 = persistences.length; _l < _len1; i = ++_l) {
        persistence = persistences[i];
        _ref = octaves[i], w = _ref.w, h = _ref.h, pixels = _ref.pixels;
        if (i === 0) {
